	...
	public Envelope call(String line) throws Exception {
		...
		switch (splitter) {
			case "csv":
				...
				x1 = Double.parseDouble(lineSplitList.get(offset)); //longitude1
				x2 = Double.parseDouble(lineSplitList.get(offset + 2)); //longitude2
				y1 = Double.parseDouble(lineSplitList.get(offset + 1)); //latitude1
				y2 = Double.parseDouble(lineSplitList.get(offset + 3)); //latitude2
